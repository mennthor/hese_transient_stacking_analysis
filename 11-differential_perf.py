...
"""
Single job of differential performance trials.

Almost the same as for the `09-performance.py` script, except we take another
parameter specifying the energy range the signal injector is allowed to inject.
This way we get the performance differential in true neutrino energies.
"""
...
parser.add_argument("--log10_E_nu_lo", type=float)
parser.add_argument("--log10_E_nu_hi", type=float)
args = parser.parse_args()
rnd_seed = args.rnd_seed
ntrials = args.ntrials
tw_id = args.tw_id
sig_inj_type = args.sig_inj
log_E_nu_lo = args.log10_E_nu_lo
log_E_nu_hi = args.log10_E_nu_hi

if log_E_nu_lo >= log_E_nu_hi:
    raise ValueError("`log_E_nu_lo` must be smaller than `log_E_nu_hi`.")
...
for key in sample_names:
    ...
    if sig_inj_type == "healpy":
        ...
        sig_inj_i = HealpySignalFluenceInjector(
            ...
            random_state=rndgen)
        sig_inj_i.fit(srcs_rec, src_maps=src_maps, MC=mc[mask])
        del src_maps
...
out = {
    ...
    "log_E_nu_lo": log_E_nu_lo,
    "log_E_nu_hi": log_E_nu_hi,
    }

# Save as JSON
outpath = os.path.join(PATHS.data, "differential_perf_trials_" + sig_inj_type)
